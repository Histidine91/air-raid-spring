...
function gadget:UnitDestroyed(u, ud, team, attacker, aud, ateam)
	if ateam --[[and GG.teamplane[ateam] and attacker == GG.teamplane[ateam].unit]] and (not Spring.AreTeamsAllied(team, ateam)) then
		...
		if score[ateam]>=nextBonus[ateam] then
			if nextBonus[ateam]>=1500 then
				nextBonus[ateam]=nextBonus[ateam]+1000
			elseif nextBonus[ateam]==500 then
				nextBonus[ateam]=1500
			else
				nextBonus[ateam]=500
			end
